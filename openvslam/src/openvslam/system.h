...
public:
    ...
    //! Startup the SLAM system
    void startup_to_set_station(const bool need_initialize = true);
    ...
    //! Feed a monocular frame to SLAM system
    //! (NOTE: distorted images are acceptable if calibrated)
    Mat44_t feed_monocular_frame_to_set_station(const cv::Mat& img, const double timestamp, const cv::Mat& mask = cv::Mat{});
