...
Mat44_t tracking_module::track_monocular_image(const cv::Mat& img, const double timestamp, const cv::Mat& mask) {
    ...
    }                                       //ini_extractor_left_ == ORB extractor only when used in initializing
    else {//3.Tracking 4.Lost
        curr_frm_ = data::frame(img_gray_, timestamp, extractor_left_, bow_vocab_, camera_, cfg_->true_depth_thr_, mask);
    }                                       //extractor_left_ == ORB extractor for left/monocular image
...
Mat44_t tracking_module::track_monocular_image_and_set_station(const cv::Mat& img, const double timestamp, const cv::Mat& mask) {
    const auto start = std::chrono::system_clock::now();
    // color conversion
    img_gray_ = img;
    util::convert_to_grayscale(img_gray_, camera_->color_order_);
    ...
    if(station_saved_count==0)
    {    station_saved_count+=1;
        mapping_station_with_current_frame();
    }
...
void tracking_module::mapping_station_with_current_frame() {

    store_new_station();
}

void tracking_module::store_new_station() {

    const Vec3_t cur_cam_center = curr_frm_.get_cam_center();

    auto st = new data::station(cur_cam_center, map_db_);
    // store the new station to the map database
    map_db_->add_station(st);
}
...
void tracking_module::track() {
    ...
    if (tracking_state_ == tracker_state_t::Initializing) {
        if (!initialize()) { // bool initialize == try to initialize with the current frame
            return;
        }
    ...
    else {    // startup(false) is tracker_state_t::Lost
...
bool tracking_module::track_current_frame() {
    ...
    if (tracking_state_ == tracker_state_t::Tracking) {
        ...
        // Lost mode
        // try to relocalize  startup(false) 
