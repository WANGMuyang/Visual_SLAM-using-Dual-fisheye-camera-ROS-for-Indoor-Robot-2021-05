...
public:
    ...
    //! Track a monocular frame
    //! (NOTE: distorted images are acceptable if calibrated)
    Mat44_t track_monocular_image_and_set_station(const cv::Mat& img, const double timestamp, const cv::Mat& mask = cv::Mat{});
...
protected:
    ...
    //! Create station and extend the map with current frame
    void mapping_station_with_current_frame();

    //! Store the new station to the map database
    void store_new_station();
