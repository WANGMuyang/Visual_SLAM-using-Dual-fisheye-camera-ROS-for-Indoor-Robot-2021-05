...
void map_database_io::save_message_pack(const std::string& path) {
    ...
    map_db_->to_json(keyfrms, landmarks, stations);
    ...
    nlohmann::json json{{"cameras", cameras},
                        ...
                        {"station_next_id", static_cast<unsigned int>(data::station::next_id_)}};
...
void map_database_io::load_message_pack(const std::string& path) {
    ...
    data::station::next_id_ = json.at("station_next_id").get<unsigned int>(); //error???
    // load database
    ...
    const auto json_stations = json.at("stations");
    map_db_->from_json(cam_db_, bow_vocab_, bow_db_, json_keyfrms, json_landmarks, json_stations);
