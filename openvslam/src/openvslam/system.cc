...
void system::startup(const bool need_initialize) {
    ...
    if (!need_initialize) { //need_initialize == true이면 여기는 동작안함 -> run_slam
        tracker_->tracking_state_ = tracker_state_t::Lost;
        // spdlog::info("startup SLAM system2");
    }
...
void system::startup_to_set_station(const bool need_initialize) {
    spdlog::info("startup SLAM system");
    system_is_running_ = true;

    if (!need_initialize) { //need_initialize == false이면 여기는 동작함 -> set_station_locatization of run_localization
        tracker_->tracking_state_ = tracker_state_t::Lost;
        // spdlog::info("startup SLAM system3");
    }

    mapping_thread_ = std::unique_ptr<std::thread>(new std::thread(&openvslam::mapping_module::run_to_set_station, mapper_));
    global_optimization_thread_ = std::unique_ptr<std::thread>(new std::thread(&openvslam::global_optimization_module::run, global_optimizer_));
}
...
Mat44_t system::feed_monocular_frame(const cv::Mat& img, const double timestamp, const cv::Mat& mask) {
    ...
    const Mat44_t cam_pose_cw = tracker_->track_monocular_image(img, timestamp, mask); // Eigen::Matrix4d;

    frame_publisher_->update(tracker_); // after track_monocular_image,tracker_state_t change Lost to Tracking
    if (tracker_->tracking_state_ == tracker_state_t::Tracking) {
        map_publisher_->set_current_cam_pose(cam_pose_cw);
    }

    return cam_pose_cw; //Eigen::Matrix4d;
}

Mat44_t system::feed_monocular_frame_to_set_station(const cv::Mat& img, const double timestamp, const cv::Mat& mask) {
    assert(camera_->setup_type_ == camera::setup_type_t::Monocular);

    check_reset_request();

    const Mat44_t cam_pose_cw = tracker_->track_monocular_image_and_set_station(img, timestamp, mask); // Eigen::Matrix4d;
