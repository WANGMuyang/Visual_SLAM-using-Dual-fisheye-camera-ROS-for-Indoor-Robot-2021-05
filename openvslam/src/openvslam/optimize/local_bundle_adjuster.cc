...
void local_bundle_adjuster::optimize(openvslam::data::keyframe* curr_keyfrm, bool* const force_stop_flag) const {
    // 1. local/fixed keyframes, local landmarks을 집계하다
    ...
    for (auto local_lm : local_lms) {
        ...
        for (auto& obs : observations) {
            ...
            // local keyframes에 속해 있을 때는 추가하지 않음
    ...
    // fixed keyframes를 optimizer에 세트
    ...
    // 4. keyframe과 landmark의 vertex를 reprojection edge로 접속하는
    ...
    for (auto& id_local_lm_pair : local_lms) {
        ...
        // landmark를 g2o의 vertex로 변환하여 optimizer에 세트한다.
