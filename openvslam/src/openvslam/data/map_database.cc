...
void map_database::add_station(station* st) {
    std::lock_guard<std::mutex> lock(mtx_map_access_);
    stations_[st->id_] = st;
}

void map_database::erase_station(station* st) {
    std::lock_guard<std::mutex> lock(mtx_map_access_);
    stations_.erase(st->id_);
...
std::vector<station*> map_database::get_all_stations() const {
    std::lock_guard<std::mutex> lock(mtx_map_access_);
    std::vector<station*> stations;
    stations.reserve(stations.size());
    for (const auto id_station : stations_) {
        stations.push_back(id_station.second);
    }
    return stations;
}
...
unsigned int map_database::get_num_stations() const {
    std::lock_guard<std::mutex> lock(mtx_map_access_);
...
void map_database::from_json(camera_database* cam_db, bow_vocabulary* bow_vocab, bow_database* bow_db,
                             ...
                             const nlohmann::json& json_stations) {
    std::lock_guard<std::mutex> lock(mtx_map_access_);
    ...
    // Step 3.5. Register 3D station point
    // If the object does not exist at this step, the corresponding pointer is set as nullptr.
    spdlog::info("decoding {} statioins to load", json_stations.size());
    for (const auto& json_id_station : json_stations.items()) {
        const auto id = std::stoi(json_id_station.key());
        assert(0 <= id);
        const auto json_station = json_id_station.value();

        register_station(id, json_station);
    }
...
void map_database::register_station(const unsigned int id, const nlohmann::json& json_station) { //error!!!
    // const Vec3_t pos_w = convert_json_to_translation(json_station.at("pos_w"));
    // const auto pos_w = Vec3_t(json_station.at("pos_w")[id].get<std::vector<Vec3_t::value_type>>().data());
    const auto pos_w = Vec3_t(json_station.at("pos_w").get<std::vector<Vec3_t::value_type>>().data());
    //const Vec3_t pos_w = json_station.at("pos_w");
    auto st = new data::station(pos_w, id, this);
    assert(!stations_.count(st));
    stations_[st->id_] = st;
}
...
void map_database::to_json(nlohmann::json& json_keyfrms, nlohmann::json& json_landmarks,nlohmann::json& json_stations) {
    std::lock_guard<std::mutex> lock(mtx_map_access_);
    ...
    // Save each 3D staion as json
    spdlog::info("encoding {} stations to store", stations_.size());
    std::map<std::string, nlohmann::json> stations;
    for (const auto id_st : stations_) {
        const auto id = id_st.first;
        const auto st = id_st.second;
        assert(st);
        assert(id == st->id_);
        assert(!stations.count(std::to_string(id)));
        stations[std::to_string(id)] = st->to_json();
    }
    json_stations = stations;
}
