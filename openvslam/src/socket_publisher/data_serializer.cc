...
data_serializer::data_serializer(const std::shared_ptr<openvslam::publish::frame_publisher>& frame_publisher,
    ...
    : frame_publisher_(frame_publisher), map_publisher_(map_publisher),
      ...
      station_hash_map_(new std::unordered_map<unsigned int, double>){
...
std::string data_serializer::serialize_map_diff() {
    ...
    std::vector<openvslam::data::station*> all_stations;
    map_publisher_->get_landmarks(all_landmarks, local_landmarks);
    map_publisher_->get_station(all_stations);
    ...
    return serialize_as_protobuf(keyframes, all_landmarks, local_landmarks, all_stations, current_camera_pose);
}
...
std::string data_serializer::serialize_as_protobuf(const std::vector<openvslam::data::keyframe*>& keyfrms,
                                                   ...
                                                   const std::vector<openvslam::data::station*>& all_stations,
    ...
    *point_hash_map_ = next_point_hash_map;
    ...
    // 4.5 staion registration
    std::unordered_map<unsigned int, double> next_station_hash_map;
    for (const auto station : all_stations) {

        const auto id = station->id_;
        const auto pos = station->get_pos_of_station_in_world();
        const auto zip = get_vec_hash(pos);

        // point exists on next_point_zip.
        next_station_hash_map[id] = zip;

        // remove point from point_zip.
        if (station_hash_map_->count(id) != 0) {
            if (station_hash_map_->at(id) == zip) {
                station_hash_map_->erase(id);
                continue;
            }
            station_hash_map_->erase(id);
        }
        const unsigned int rgb2[] = {0, 255, 0};

        // add to protocol buffers
        auto station_obj = map.add_stations();
        station_obj->set_id(id);
        for (int i = 0; i < 3; i++) {
            station_obj->add_coords(pos[i]);
        }
        for (int i = 0; i < 3; i++) {
            station_obj->add_color(rgb2[i]);
        }
    }
    // removed points are remaining in "point_zips".
    for (const auto& itr : *station_hash_map_) {
        const auto id = itr.first;

        auto station_obj = map.add_stations();
        station_obj->set_id(id);
    }
    *station_hash_map_ = next_station_hash_map;
