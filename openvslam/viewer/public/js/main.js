...
const REFERENCE_STATION_COLOR = [0, 255, 0];
...
let stationUpdateFlag = false;
let stationCloud = new StationCloud();   //이거부터가 문제인거네  let pointUpdateFlag = false;가 중복 선언되기도 함
...
function init() {
    ...
    // THREE.Geometry.vertices : Array
    // Array of vertices.  vertex = 꼭지점
...
function render() {
    ...
    stationCloud.updateStationInScene(scene);
...
function initGui() {
    ...
    gui.add(property, 'StationSize', 0, 4, 0.1).onChange(setStationSize);
...
function setStationSize(val) {
    val = Math.pow(2, val);
    stationCloud.setStationSize(val); //station_size
}
...
function loadProtobufData(obj, keyframes, edges, points, stations, referencePointIds, currentFramePose) {
    ...
    for (let StationObj of obj.stations) {
        let Station = {};
        Station["id"] = StationObj.id;
        if (StationObj.coords.length != 0) {
            Station["point_pos"] = StationObj.coords;
            Station["rgb"] = StationObj.color;
        }
...
function receiveProtobuf(msg) {
    ...
    if (obj.messages[0].tag == "RESET_ALL") {
        ...
        loadProtobufData(obj, keyframes, edges, points, stations, referencePointIds, currentFramePose);
        updateMapElements(msg.length, keyframes, edges, points, stations, referencePointIds, currentFramePose);
    }
}
...
function updateMapElements(msgSize, keyframes, edges, points, stations, referencePointIds, currentFramePose) {
    trackStats.update();
    ...
    for (let station of stations) {
        let id = station["id"];
        let x = station["point_pos"][0] * GLOBAL_SCALE;
        let y = station["point_pos"][1] * GLOBAL_SCALE;
        let z = station["point_pos"][2] * GLOBAL_SCALE;
        let r = station["rgb"][0];
        let g = station["rgb"][1];
        let b = station["rgb"][2];
        stationCloud.updateStation(id, x, y, z, r, g, b);
...
function removeAllElements() {
    ...
    for (let id in stationCloud.vertexIds) {
        if (id < 0 || id == undefined) {
            continue;
        }
        stationCloud.removeStation(id);
    }
