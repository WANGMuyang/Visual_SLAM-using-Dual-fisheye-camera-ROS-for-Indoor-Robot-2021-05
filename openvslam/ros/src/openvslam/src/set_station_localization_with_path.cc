...
void station_localization(const std::shared_ptr<openvslam::config>& cfg, const std::string& vocab_file_path,
    ...
    SLAM.startup_to_set_station(false);
    ...
    cv::Mat frame;
    frame = cv::imread("/home/alpha/openvslam/stations/station.png");
    ...
    SLAM.feed_monocular_frame_to_set_station(frame, timestamp, mask);
    ...
    //     // input the current frame and estimate the camera pose
    //     SLAM.feed_monocular_frame_to_set_station(cv_bridge::toCvShare(msg, "bgr8")->image, timestamp, mask);
...
int main(int argc, char* argv[]) {
...
#endif
    ros::init(argc, argv, "set_station_localization");
    ...
    // set_station_localization
    if (cfg->camera_->setup_type_ == openvslam::camera::setup_type_t::Monocular) {
        station_localization(cfg, vocab_file_path->value(), mask_img_path->value(), map_db_path->value(), mapping->is_set());
    }
    else {
